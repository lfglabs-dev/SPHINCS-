/-
  ...
  It is ADDITIVE relative to the executable bridge: it defines the concrete
  primitive package, while `ProofCore.lean` wires that package to the concrete
  `execC12` runner and `Proofs.lean` keeps the bridge axiom.
-/
...
/-- The concrete C12 parser has exactly one guard: signature byte length.  On
the length-ok branch it always constructs a parsed signature. -/
theorem parseSignatureC12_some_of_size {v : Variant} {sig : Bytes}
    (hsz : sig.size = v.sigBytes) :
    ∃ s, parseSignatureC12 v sig = some s := by
...
theorem parseSignatureC12_eq_none_iff {v : Variant} {sig : Bytes} :
    parseSignatureC12 v sig = none ↔ sig.size ≠ v.sigBytes := by
  unfold parseSignatureC12
  by_cases hsz : sig.size = v.sigBytes <;> simp [hsz]

/-- `Option.isSome` form of `parseSignatureC12_some_of_size`. -/
theorem parseSignatureC12_isSome_iff {v : Variant} {sig : Bytes} :
    (parseSignatureC12 v sig).isSome ↔ sig.size = v.sigBytes := by
  unfold parseSignatureC12
  by_cases hsz : sig.size = v.sigBytes <;> simp [hsz]

/-- C12 accepts the contract-facing public-key words without low-byte
canonicality checks. -/
theorem publicKeyOk_c12 (pk : PublicKey) :
    publicKeyOk c12 pk = true := by
  simp [publicKeyOk, c12]

/-- Byte-level C12 public-key parsing is just the two exposed `bytes32`
arguments packaged as a `PublicKey`. -/
theorem parsePublicKey_c12 (pkSeed pkRoot : Bytes) :
    SphincsMinusVerifierSpec.ByteLevel.parsePublicKey c12 pkSeed pkRoot =
      some { pkSeed := pkSeed, pkRoot := pkRoot } := by
  simp [SphincsMinusVerifierSpec.ByteLevel.parsePublicKey, publicKeyOk_c12]

/-- Current byte-level C12 public-key parsing, like C13's, accepts public-key
root byte arrays without enforcing the 16-byte root shape used by the concrete
word-level runner. -/
theorem parsePublicKey_c12_does_not_force_pkRoot_size :
    ∃ pkSeed pkRoot pk,
      SphincsMinusVerifierSpec.ByteLevel.parsePublicKey c12 pkSeed pkRoot =
        some pk ∧
  ...
  refine ⟨ByteArray.empty, ByteArray.empty,
    ...
    ?_, ?_⟩
  · exact parsePublicKey_c12 ByteArray.empty ByteArray.empty
  · simp [ByteArray.size]
...
/-- C12 WOTS public-key reconstruction at a concrete hypertree layer. -/
def wotsPkFromSigC12AtLayer (layer : Nat) (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (node : Bytes) (wots : WotsSig) : Option Bytes :=
  let seed := wordOfHash16 pk.pkSeed
  let nodeW := wordOfHash16 node
  some (hash16OfWord (wotsPkWordC12 seed layer treeIdx leafIdx nodeW wots))

/-- Never consulted for C12 (`standardChecksum`); returns `true`. -/
def wotsGrindingOkC12AtLayer (layer : Nat) (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (node : Bytes) (wots : WotsSig) : Bool := true

/-- Contract-facing layer-0 C12 WOTS reconstruction. -/
def wotsPkFromSigC12 (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (node : Bytes) (wots : WotsSig) : Option Bytes :=
  wotsPkFromSigC12AtLayer 0 v pk treeIdx leafIdx node wots

/-- Contract-facing layer-0 C12 grinding predicate. -/
def wotsGrindingOkC12 (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (node : Bytes) (wots : WotsSig) : Bool :=
  wotsGrindingOkC12AtLayer 0 v pk treeIdx leafIdx node wots
...
/-- C12 XMSS root reconstruction at a concrete hypertree layer. -/
def xmssRootFromSigC12AtLayer (layer : Nat) (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (wotsPk : Bytes) (auth : List Bytes) : Option Bytes :=
  let seed := wordOfHash16 pk.pkSeed
  let xmssBase := xmssBaseC12 layer treeIdx
  let start := wordOfHash16 wotsPk
  let root := xmssClimb seed xmssBase 4 0 leafIdx start auth
  some (hash16OfWord root)

/-- Contract-facing layer-0 C12 XMSS reconstruction. -/
def xmssRootFromSigC12 (v : Variant) (pk : PublicKey)
    (treeIdx leafIdx : Nat) (wotsPk : Bytes) (auth : List Bytes) : Option Bytes :=
  xmssRootFromSigC12AtLayer 0 v pk treeIdx leafIdx wotsPk auth
...
def c12PrimitivesConcrete : Primitives :=
  ...
  , wotsPkFromSigAtLayer := wotsPkFromSigC12AtLayer
  , wotsGrindingOk  := wotsGrindingOkC12
  , wotsGrindingOkAtLayer := wotsGrindingOkC12AtLayer
  , xmssRootFromSig := xmssRootFromSigC12
  , xmssRootFromSigAtLayer := xmssRootFromSigC12AtLayer }
...
#print axioms parsePublicKey_c12
#print axioms parsePublicKey_c12_does_not_force_pkRoot_size
