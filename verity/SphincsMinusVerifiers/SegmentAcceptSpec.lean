...
/-- Range-gated concrete layer obligations after the concrete FORS root-cell
bridge is discharged from `mkC13State`.  This is the satisfiable replacement for
`C13SeedNamedAcceptConcreteLayerObligations`: callers provide concrete WOTS/XMSS
and model `"merkleNode"` facts only for the two layer-loop indices visited by
C13. -/
structure C13SeedNamedAcceptConcreteLayerRangeObligations
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) : Prop where
  hGuard : ∀ (s : RuntimeState) (node : ByteArray) (idx : Nat), idx < 2 →
    CurrentNodeRel wordOfHash16 s node →
...
/-- Range-gated concrete layer obligations where the executable digit cell is
related to the concrete WOTS digest sum.  Together with the existing
`wotsGrindingFails = false` success fact, this derives the guard target `208`. -/
structure C13SeedNamedAcceptConcreteLayerDigitCellRangeObligations
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) : Prop where
  hDigitCell :
    ∀ (s : RuntimeState) (node : ByteArray) (idx : Nat) (lsig : XmssLayerSig),
      idx < 2 →
      CurrentNodeRel wordOfHash16 s node →
      sigParsed.layers[idx]? = some lsig →
        lookupValue
        ...
        C13Concrete.wotsDigitSum
          (C13Concrete.wotsDigest (C13Concrete.wordOfHash16 pkSeed) 0
            ...
            (c13LayerLeafIdx
              (C13Concrete.c13PrimitivesConcrete.hMsg c13
                { pkSeed := pkSeed, pkRoot := pkRoot } sigParsed.R message) idx)
            lsig.wots.count (C13Concrete.wordOfHash16 node))
  hSuccess : ∀ (s : RuntimeState) (node : ByteArray) (idx : Nat), idx < 2 →
    CurrentNodeRel wordOfHash16 s node →
      ∃ (lsig : XmssLayerSig) (wotsPk root : ByteArray),
        sigParsed.layers[idx]? = some lsig ∧
...
/-- Convert the executable digit-cell correspondence plus concrete grinding
success into the digit-sum target package consumed by the guard handoff. -/
theorem digitSum_range_obligations_of_digit_cell_range_obligations
    (pkSeed pkRoot message sig : ByteArray) (sigParsed : Signature)
    (hObs : C13SeedNamedAcceptConcreteLayerDigitCellRangeObligations
      pkSeed pkRoot message sig sigParsed) :
    C13SeedNamedAcceptConcreteLayerDigitSumRangeObligations
      pkSeed pkRoot message sig sigParsed := by
  exact
    { hDigitSum := by
        intro s node idx hidx hRel
        rcases hObs.hSuccess s node idx hidx hRel with
          ⟨lsig, _wotsPk, _root, hLayer, hGrinding, _hWots, _hXmss, _hMerkleNode⟩
        rw [hObs.hDigitCell s node idx lsig hidx hRel hLayer]
        exact C13Concrete.wotsGrindingFailsC13_false_digitSum
          { pkSeed := pkSeed, pkRoot := pkRoot }
          (c13LayerNextTree
            (C13Concrete.c13PrimitivesConcrete.hMsg c13
              { pkSeed := pkSeed, pkRoot := pkRoot } sigParsed.R message) idx)
...
/-- Adapter from concrete C13 layer/FORS-site obligations to the abstract
site-root accept bundle.  This removes the abstract `specStep`,
`LayerGuardedStep`, and `hSpecFold` fields by instantiating them with the
concrete C13 hypertree step and successful concrete `foldHypertree` result. -/
theorem site_root_obligations_of_concrete_layer_site_root_obligations
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
  ...
  exact
    { hLayerGuardStep := by
        simpa [pk, digest] using
          layerGuardedStep_c13HypertreeSpecStep_of_success
            pk digest sigParsed.layers hObs.hGuard hObs.hSuccess
      hSite := hObs.hSite
      hNode := by
        intro j hj
        simpa [pk, digest] using hObs.hNode j hj
      hSpecFold := by
        simpa [pk, digest] using
          specFold_c13HypertreeSpecStep_eq_of_foldHypertree_ok
            pk digest forsPk specRoot sigParsed.layers hFold
      hPkRootSize := hObs.hPkRootSize }

/-- Adapter from concrete layer obligations to the guarded accept bundle with
seed and FORS root-cell facts.  The FORS root cells are derived from the
concrete C13 root-cell theorem rather than from caller-supplied FORS site/node
obligations. -/
theorem seed_named_pk_root_size_obligations_of_concrete_layer_obligations
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
    (hParse : C13Concrete.parseSignatureC13 c13 sig = some sigParsed)
    (hFold : foldHypertree C13Concrete.c13PrimitivesConcrete c13
        ...
        (C13Concrete.c13PrimitivesConcrete.hMsg c13
          { pkSeed := pkSeed, pkRoot := pkRoot } sigParsed.R message)
        forsPk sigParsed.layers = .ok specRoot)
    (hObs : C13SeedNamedAcceptConcreteLayerObligations
        pkSeed pkRoot message sig sigParsed) :
    C13SeedNamedAcceptGuardedPkRootSizeObligations
        pkSeed pkRoot message sig sigParsed forsPk specRoot
        (c13HypertreeSpecStep { pkSeed := pkSeed, pkRoot := pkRoot }
...
/-- Byte-shaped concrete C13 accept handoff with layer facts stated only over
the actual two-iteration hypertree loop.  This is the range-gated replacement for
`accept_path_returns_verifyParsed_bool_from_concrete_layer_obligations_of_bytes`:
the theorem derives the layer guard trace and final `"currentNode"` fact from
`idx < 2` guard/success premises instead of requiring an impossible all-`Nat`
layer package. -/
theorem accept_path_returns_verifyParsed_bool_from_concrete_layer_range_obligations_of_bytes
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
    (hParse : C13Concrete.parseSignatureC13 c13 sig = some sigParsed)
    (hZero : forcedZeroOk c13
    ...
    (hFold : foldHypertree C13Concrete.c13PrimitivesConcrete c13
        ...
        (C13Concrete.c13PrimitivesConcrete.hMsg c13
          { pkSeed := pkSeed, pkRoot := pkRoot } sigParsed.R message)
        forsPk sigParsed.layers = .ok specRoot)
    (hObs : C13SeedNamedAcceptConcreteLayerRangeObligations
        pkSeed pkRoot message sig sigParsed) :
    ∃ finalState,
      verifyParsed C13Concrete.c13PrimitivesConcrete c13
        { pkSeed := pkSeed, pkRoot := pkRoot } message sigParsed
  ...
  have hLen : lookupValue (mkC13State pkSeed pkRoot message sig).bindings "sig_length"
        = wordNormalize 3688 :=
    c13_sig_length_of_parseSignatureC13 pkSeed pkRoot message sig sigParsed hParse
  have hS3 : SegmentS3.s3Guard (afterS2 (mkC13State pkSeed pkRoot message sig)) = 0 :=
    c13_s3Guard_of_parse_forcedZero pkSeed pkRoot message sig pk sigParsed
      (by rfl) hParse (by simpa [pk, digest] using hZero)
  have hRoots :=
    CurrentNodeFrame.rootCells_eq_forsAllRootsC13_of_hMsg_parse_concrete
      pk message sig hParse
  have hStart : CurrentNodeRel wordOfHash16
      { (afterSeed (mkC13State pkSeed pkRoot message sig)) with
          bindings := bindValue
            (afterSeed (mkC13State pkSeed pkRoot message sig)).bindings
            "layer" (wordNormalize 0) }
      forsPk := by
    refine layerStart_of_seed_named_fors_roots_roundtrip
      pkSeed pkRoot message sig pk sigParsed forsPk (by rfl)
      (by simpa [pk, digest] using hFors)
      (CurrentNodeFrame.afterFors_seed_slot_mkC13State pkSeed pkRoot message sig)
      ?_ ?_ ?_
    · intro j hj
      simpa [pk, digest] using hRoots.1 j hj
    · simpa [pk, digest] using hRoots.2
    · simpa [pk, digest] using
        (forsPkWordC13_roundtrip pk digest sigParsed.fors)
  have hSuccessRange :
      ∀ (s : RuntimeState) (node : ByteArray) (idx : Nat), idx < 2 →
        CurrentNodeRel wordOfHash16 s node →
          ∃ (lsig : XmssLayerSig) (wotsPk root : ByteArray),
            ...
            lookupValue
              ...
              = wordOfHash16 root := by
    intro s node idx hidx hRel
    simpa [pk, digest] using hObs.hSuccess s node idx hidx hRel
  have hLayerGuards :
      ClimbLoopGuarded.allGuardsPass "layer" SegmentLayer3.stepLayer
        SegmentLayer3.layerGuard
        { (afterSeed (mkC13State pkSeed pkRoot message sig)) with
        ...
        0 (wordNormalize 2) :=
    layerGuardsPass_of_c13HypertreeSpecStep_success_range
      pkSeed pkRoot message sig pk digest sigParsed.layers forsPk hStart
      (by
        intro s node idx hidx hRel
        exact hObs.hGuard s node idx hidx hRel)
      hSuccessRange
  have hLayerStep :=
    layerStep_of_c13HypertreeSpecStep_success_range
      pk digest sigParsed.layers hSuccessRange
  have hSpecFold :
      ClimbLoop.specFold (c13HypertreeSpecStep pk digest sigParsed.layers)
        forsPk 0 (wordNormalize 2) = specRoot := by
    simpa [pk, digest] using
      specFold_c13HypertreeSpecStep_eq_of_foldHypertree_ok
  ...
  have hStartLookup :
      lookupValue (afterSeed (mkC13State pkSeed pkRoot message sig)).bindings "currentNode"
        = wordOfHash16 forsPk := by
    have hs := hStart
    unfold CurrentNodeRel at hs
    rw [MemoryKit.lookupValue_bindValue_ne
      (afterSeed (mkC13State pkSeed pkRoot message sig)).bindings
      "layer" "currentNode" (wordNormalize 0) (by decide)] at hs
    exact hs
  have hCurrent :
      lookupValue (afterLayer (mkC13State pkSeed pkRoot message sig)).bindings "currentNode"
        = wordOfHash16 specRoot :=
    CurrentNodeFrame.afterLayer_currentNode_of_step_range
      (mkC13State pkSeed pkRoot message sig)
      (c13HypertreeSpecStep pk digest sigParsed.layers) wordOfHash16 forsPk specRoot
      hStartLookup hLayerStep hSpecFold
  have hWordCmp : decide (wordOfHash16 specRoot = wordOfHash16 pkRoot)
    ...
    wordCmp_of_wordOfHash16_roundtrip specRoot pkRoot
      (specRoot_roundtrip_of_c13_fors_fold
        (by simpa [pk, digest] using hFors)
        (by simpa [pk, digest] using hFold))
      (hash16OfWord_wordOfHash16_of_size pkRoot hObs.hPkRootSize)
  exact accept_path_returns_verifyParsed_bool_from_root
    pkSeed pkRoot message sig pk sigParsed forsPk specRoot (by rfl)
    hShape (by simpa [pk, digest] using hZero)
    (by simpa [pk, digest] using hFors)
    (by simpa [pk, digest] using hFold)
...
/-- Byte-shaped concrete C13 accept handoff where the layer guard follows from
the executable digit-cell correspondence and the concrete WOTS grinding success
already present in the range success fact. -/
theorem accept_path_returns_verifyParsed_bool_from_concrete_layer_digit_cell_range_obligations_of_bytes
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
    (hParse : C13Concrete.parseSignatureC13 c13 sig = some sigParsed)
    (hZero : forcedZeroOk c13
  ...
  exact
    accept_path_returns_verifyParsed_bool_from_concrete_layer_digitSum_range_obligations_of_bytes
      pkSeed pkRoot message sig sigParsed forsPk specRoot
      hParse hZero hFors hFold
      (digitSum_range_obligations_of_digit_cell_range_obligations
        pkSeed pkRoot message sig sigParsed hObs)
...
`pkRoot.size = 16`; the theorem derives the abstract layer step, layer fold,
range-gated seed frame, and all FORS root-cell facts internally. -/
theorem accept_path_returns_verifyParsed_bool_from_concrete_layer_site_root_obligations_of_bytes
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
    (hParse : C13Concrete.parseSignatureC13 c13 sig = some sigParsed)
    (hZero : forcedZeroOk c13
  ...
  exact
    accept_path_returns_verifyParsed_bool_from_seed_named_guarded_pk_root_size_site_root_obligations_of_bytes
      ...
      (c13HypertreeSpecStep { pkSeed := pkSeed, pkRoot := pkRoot }
        ...
        sigParsed.layers)
      hParse hZero hFors hFold
      (site_root_obligations_of_concrete_layer_site_root_obligations
        pkSeed pkRoot message sig sigParsed forsPk specRoot hFold hObs)

/-- Byte-shaped guarded handoff after the concrete FORS root-cell bridge.  The
caller supplies only concrete layer guard/success facts, parse/spec facts, and
`pkRoot.size = 16`; the theorem derives the abstract layer step, layer fold,
seed cell, and all seven FORS root-cell facts internally. -/
theorem accept_path_returns_verifyParsed_bool_from_concrete_layer_obligations_of_bytes
    (pkSeed pkRoot message sig : ByteArray)
    (sigParsed : Signature) (forsPk specRoot : ByteArray)
    (hParse : C13Concrete.parseSignatureC13 c13 sig = some sigParsed)
    (hZero : forcedZeroOk c13
  ...
  exact
    accept_path_returns_verifyParsed_bool_from_seed_named_guarded_pk_root_size_obligations_of_bytes
      ...
      (seed_named_pk_root_size_obligations_of_concrete_layer_obligations
        pkSeed pkRoot message sig sigParsed forsPk specRoot hParse hFold hObs)
...
#print axioms C13SeedNamedAcceptConcreteLayerDigitCellRangeObligations
#print axioms digitSum_range_obligations_of_digit_cell_range_obligations
...
#print axioms site_root_obligations_of_concrete_layer_site_root_obligations
#print axioms seed_named_pk_root_size_obligations_of_concrete_layer_obligations
...
#print axioms accept_path_returns_verifyParsed_bool_from_concrete_layer_digit_cell_range_obligations_of_bytes
